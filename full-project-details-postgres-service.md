...
# Good addition — right now you can only restore from backups your platform created. Let's add "Restore from local file", letting users upload their own .sql file directly.

# 1. Install multer (handles file uploads) in the backend
powershellcd backend
npm install multer
# 2. Update backend/src/routes/backups.js

Add these at the top, alongside your other imports:

jsconst multer = require('multer');
const upload = multer({ dest: '/tmp/uploads/' });
Add this new route, before module.exports = router;:
js// RESTORE from an uploaded local file
...
Note: this uses execFile (not exec) — same fix as your earlier pg_dump issue, avoiding Windows/
shell path parsing problems.

# 3. Add multer's temp folder to .dockerignore exceptions — no change needed
/tmp/uploads/ is inside the container's filesystem, not your project folder, so nothing to add here.

# 4.  Frontend: Add upload UI to DatabaseDetail.jsx
...
const DatabaseDetail = () => {
  ...
  const handleUploadRestore = async () => {
    ...
    if (!window.confirm(`Restore from "${uploadFile.name}"? This runs the file's SQL against the live database.`)) return
    ...
    try {
      ...
      await apiClient.post(`/backups/${dbName}/restore-upload`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      })
      alert('Restore complete')
      setUploadFile(null)
    } catch (err) {
      setError(err.response?.data?.error || 'Upload restore failed')
    } finally {
      setUploading(false)
    }
  }
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
            <input
              ...
              className="text-sm text-gray-600 file:mr-3 file:py-2 file:px-3 file:rounded file:border file:border-gray-300 file:bg-gray-50 file:text-gray-700 file:text-sm file:cursor-pointer hover:file:bg-gray-100"
            />
            <button
              onClick={handleUploadRestore}
              disabled={!uploadFile || uploading}
              className="text-sm bg-white border border-gray-300 text-gray-700 px-4 py-2 rounded hover:bg-gray-50 disabled:opacity-40 shrink-0"
            >
